...
public class SnowAnimation {
    ...
    public void tick(MinecraftClient client, int width, int height, Identifier[] source) {
        this.source = source;
        ...
        for (Snow snow : this.snows) {
            RenderHelper.renderParticle(client, snow.x, height - snow.y, 4, 4, this.source[snow.source % this.source.length]);
            snow.move();
            if (snow.y > height) {
                snow.removed = true;
            }
